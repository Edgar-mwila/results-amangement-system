...
export const Route = createFileRoute('/$school/teacher/communication/')({
...
const CommunicationPage = () => {
  ...
  const handleSendEmail = () => {
    ...
    alert(
      `Email sent to ${selectedPupil.parent} regarding ${selectedPupil.name}`,
    )
    setSelectedPupil(null)
    setSubject('')
    setMessage('')
            ...
            <ul className="divide-y divide-gray-200">
              {receivedEmails.map((email) => (
                ...
                >
                  <div className="flex justify-between">
                    <span
                      className={`font-medium ${!email.read ? 'text-blue-600' : ''}`}
                    >
                      {email.from}
                      {!email.read && (
